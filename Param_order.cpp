...
//void Record_corr(FILE* output_corr, double dx, vector<double> Correlation, long N_corr, double factor); // Density correlations
...
void InitialConditions(int N, double Lx, double Ly, double radius, vector<vector<site>> &sites, vector<particle> &Particles, MTRand &ran);

// Compute forces between neighboring particles
void add_force(int k1, particle p1, int k2, particle p2, double radius, double lambda, double amplitude_r, double amplitude_phi, double Ly, double Lx, vector<force> &Force);

// Implement the dynamics
double MoveParticles(int N, int Nx, int Ny, double Lx, double Ly, vector<double> &Noise_r, vector<double> &Noise_phi, double omega, double radius, double lambda, double amplitude_r, double amplitude_phi, double mu_r, double mu_phi, vector<vector<site>> &sites,vector<particle> &Particles, double dt, double Time, vector<force> &Force);

// Analyze statistics
//void Analysis_dens(int N, int Nx, int Ny, double Lx, double Ly, double radius, double lambda, vector<force> &Forces, vector<vector<double>> &Density, int idx_max); // Local density
//void add_density(particle p, double radius, double lambda, int i, int j, int idx_max, double Ly, double Lx, vector<vector<double>> &Density); // Compute overlap
//void Analysis_corr(int N, double Lx, double Ly, double radius, vector<particle> &Particles, double dx, vector<double> Correlation); // Density correlations
...
int main(int argc, char *argv[]){ 
  ...
  */
  ...
  double amplitude_r, amplitude_phi; // Strength of interactions
...
//	double *Histogram_phi; // Array of size
  ...
  char name_snap[205]; // Name of output file containing snapshots
  ...
  char name_order[210]; // Name of output file containing order parameter
  char name_order2[210];
...
//	}
  ...
  vector<particle> Particles(2*N,p); // particles
  force f; // generic starter force
  vector<force> Force(2*N,f); // forces on the particles

  // Initialize density histogram
//	rho_max        = 2*rho;
//	N_dens         = (int) (rho_max/drho);
//	idx_max        = (int) (dist/radius);
...
//	N_phi         = (int) (2*PI/dphi);
 ...
 // memset(Histogram_phi, 0, N_phi*sizeof(double));
  ...
  // Initial conditions
  InitialConditions(N, Lx, Ly, radius, sites, Particles,ran);
  ...
  while(Time<totaltime){
    ...
    for (k=0;k<N;k++){
    		...
    		//	Noise_r[2*k]   = sqrt(2*mu_r*T)*(genRand(&ran)*2-1.0);
		//	Noise_r[2*k+1] = sqrt(2*mu_r*T)*(genRand(&ran)*2-1.0);
		//	Noise_r[2*k+1] = sqrt(2*mu_r*T)*(genRand(&ran)*2-1.0);
		//	Noise_phi[k]   = sqrt(2*mu_phi*T)*(genRand(&ran)*2-1.0);
    ...
    // Move the particles according to the dynamics
	 Time = MoveParticles(N, Nx, Ny, Lx, Ly, Noise_r, Noise_phi, omega, radius, lambda, amplitude_r, amplitude_phi, mu_r, mu_phi, sites, Particles, dt, Time, Force);
	  //printf("time: %lf \n",Time);
  ...
  //free(Sites);
  //free(Density);
  //for(i=0;i<Nx;i++)    free(Density[i]);
  //free(Noise_r);
  //free(Noise_phi);
  //free(Particles);
  //free(Force);
   // for debugging brace on while line 331 
  return 0;
}
...
double MoveParticles(int N, int Nx, int Ny, double Lx, double Ly, vector<double> &Noise_r, vector<double> &Noise_phi, double omega, double radius, double lambda, double amplitude_r, double amplitude_phi, double mu_r, double mu_phi, vector<vector<site>> &sites, vector<particle> &Particles, double dt, double Time, vector<force> &Force){
  ...
  //memset(std::vector<force> &Force, 0, N*sizeof(force));
	//vector<force> &Force(N*sizeof(force),0.);
  ...
  for(i=0;i<Nx;i++){
	...
	//			p1 = Particles[k1]; // Structure of particle 1
			...
			for(auto k2: temp_ids ){ // Loop over the particle ids
				  //if(k1==k2) continue; // No self-interaction
			...
			// Site on the right
			for(auto k2: sites[NEXT(i,Nx)][j].get_particle_ids()){ // Loop over the particle ids
				 p2 = Particles[k2]; // Get its structure
				// Site on the right
			...
			// Site on the bottom left
			for(auto k2: sites[PREV(i,Nx)][PREV(j,Ny)].get_particle_ids()){ // Loop over the particle ids	 
				  p2 = Particles[k2]; // Get its structure
			...
			//		site2 = site2->next;
				...
				// Site on the bottom 
		// Site on the bottom 
		for(auto k2: sites[i][PREV(j,Ny)].get_particle_ids()){ // Loop over the particle ids		 
				  p2 = Particles[k2]; // Get its structure
...
//					site2 = site2->next;
		...
		// Site on the bottom right
		for(auto k2: sites[NEXT(i,Nx)][PREV(j,Ny)].get_particle_ids()){ // Loop over the particle ids				  
				  p2 = Particles[k2]; // Get its structure
  ...
  for(k=0;k<N;k++){ // Iterations over the N particles
    ...
    i = (int) (Particles[k].qx/radius);
    j = (int) (Particles[k].qy/radius);
    ...
    i_new = (int) (Particles[k].qx/radius);
    j_new = (int) (Particles[k].qy/radius);
